...
- **Phase 2 foundation has landed (still no end-user behaviour change, default flag-off):** `SUPABASE_*` env vars are wired in `Config` with `SUPABASE_AUTH_ENABLED` defaulting to `false`; `services/supabase_auth_service.py` exposes a JWT verifier and a `SupabaseAdminClient` (used only by the linkage CLI); `services/supabase_auth_linkage.py` exposes the only sanctioned writers of `user.supabase_auth_user_id`; `services/auth_resolver.py` has a flag-gated Supabase JWT branch (Flask-Login still wins, never auto-links); `scripts/link_supabase_identities.py` is the admin linkage CLI (dry-run default, `--apply` required, audit-logged, reversible via `--unlink`); `decorators.bearer_or_login_required` applies the format-disambiguation policy (JWT-shaped → Supabase branch when flag on; opaque → existing `UserApiToken` path); `/admin/auth/probe` is the admin-only flag-gated read-only probe endpoint. `routes/auth_routes.py`, `forms.py`, and templates are unchanged.
- **`SUPABASE_AUTH_ENABLED` operational rule:** the flag must remain `false` in every tracked config / deployment file and in production steady state. It may be set `true` only inside a controlled staging probe window or the documented 15-minute production probe exercise, then returned to `false`. Tests mutate it on a per-test-app basis only. Do not commit `SUPABASE_AUTH_ENABLED=true` to any version-controlled file.
